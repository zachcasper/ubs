...
output "result" {
  value = {
    ...
    // UCP resource IDs
    resources = [
        "/planes/kubernetes/local/namespaces/${kubernetes_service.metadata[0].namespace}/providers/core/Service/${kubernetes_service.metadata[0].name}",
        "/planes/kubernetes/local/namespaces/${kubernetes_deployment.metadata[0].namespace}/providers/apps/Deployment/${kubernetes_deployment.metadata[0].name}"
    ]
  }
